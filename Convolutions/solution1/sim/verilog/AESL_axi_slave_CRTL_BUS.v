...
//------------------------Parameter----------------------
`define TV_IN_operation "../tv/cdatafile/c.ImageProcess.autotvin_operation.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter operation_DEPTH = 1;
reg [31 : 0] operation_OPERATE_DEPTH = 0;
parameter operation_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter ImageProcess_continue_addr = 0;
parameter ImageProcess_auto_start_addr = 0;
parameter operation_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_operation [operation_DEPTH - 1 : 0];
reg operation_write_data_finish;
...
//write operation reg
reg [31 : 0] write_operation_count = 0;
reg write_operation_run_flag = 0;
reg write_one_operation_data_done = 0;
...
assign TRAN_CRTL_BUS_write_data_finish = 1 & operation_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        operation_write_data_finish <= 0;
        write_operation_run_flag <= 0; 
        write_operation_count = 0;
        count_operate_depth_by_bitwidth_and_depth (operation_c_bitwidth, operation_DEPTH, operation_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_operation_run_flag <= 1; 
            write_operation_count = 0;
        end
        if (write_one_operation_data_done === 1) begin
            write_operation_count = write_operation_count + 1;
            if (write_operation_count == operation_OPERATE_DEPTH) begin
                write_operation_run_flag <= 0; 
                operation_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_operation
    integer write_operation_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] operation_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = operation_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_operation_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write operation data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (operation_c_bitwidth < 32) begin
                        operation_data_tmp_reg = mem_operation[write_operation_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < operation_c_bitwidth) begin
                                operation_data_tmp_reg[j] = mem_operation[write_operation_count][i*32 + j];
                            end
                            else begin
                                operation_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (operation_data_in_addr + write_operation_count * four_byte_num * 4 + i * 4, operation_data_tmp_reg, write_operation_resp);
                end
                process_busy = 0;
                write_one_operation_data_done <= 1;
                @(posedge clk);
                write_one_operation_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_operation_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [operation_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (operation_c_bitwidth , factor);
  fp = $fopen(`TV_IN_operation ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_operation); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < operation_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_operation [i/factor] = mem_tmp;
                  mem_tmp [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
