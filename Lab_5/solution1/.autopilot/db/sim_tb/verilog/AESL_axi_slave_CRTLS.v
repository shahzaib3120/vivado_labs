...
//------------------------Parameter----------------------
`define TV_IN_m1 "./c.gravity.autotvin_m1.dat"
`define TV_IN_m2 "./c.gravity.autotvin_m2.dat"
`define TV_IN_distance "./c.gravity.autotvin_distance.dat"
`define TV_OUT_ap_return "./rtl.gravity.autotvout_ap_return.dat"
...
parameter distance_DEPTH = 1;
reg [31 : 0] distance_OPERATE_DEPTH = 0;
parameter distance_c_bitwidth = 32;
...
parameter distance_data_in_addr = 40;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_m1 [m1_DEPTH - 1 : 0];
reg m1_write_data_finish;
reg [DATA_WIDTH - 1 : 0] mem_m2 [m2_DEPTH - 1 : 0];
reg m2_write_data_finish;
reg [DATA_WIDTH - 1 : 0] mem_distance [distance_DEPTH - 1 : 0];
reg distance_write_data_finish;
...
//write distance reg
reg [31 : 0] write_distance_count = 0;
reg write_distance_run_flag = 0;
reg write_one_distance_data_done = 0;
//read ap_return reg
...
assign TRAN_CRTLS_write_data_finish = 1 & m1_write_data_finish & m2_write_data_finish & distance_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        distance_write_data_finish <= 0;
        write_distance_run_flag <= 0; 
        write_distance_count = 0;
        count_operate_depth_by_bitwidth_and_depth (distance_c_bitwidth, distance_DEPTH, distance_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_CRTLS_start_in === 1) begin
            distance_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_distance_run_flag <= 1; 
            write_distance_count = 0;
        end
        if (write_one_distance_data_done === 1) begin
            write_distance_count = write_distance_count + 1;
            if (write_distance_count == distance_OPERATE_DEPTH) begin
                write_distance_run_flag <= 0; 
                distance_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_distance
    integer write_distance_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] distance_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = distance_c_bitwidth;
    process_num = 3;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_3_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_distance_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write distance data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (distance_c_bitwidth < 32) begin
                        distance_data_tmp_reg = mem_distance[write_distance_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < distance_c_bitwidth) begin
                                distance_data_tmp_reg[j] = mem_distance[write_distance_count][i*32 + j];
                            end
                            else begin
                                distance_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (distance_data_in_addr + write_distance_count * four_byte_num * 4 + i * 4, distance_data_tmp_reg, write_distance_resp);
                end
                process_busy = 0;
                write_one_distance_data_done <= 1;
                @(posedge clk);
                write_one_distance_data_done <= 0;
            end   
            process_3_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_distance_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [distance_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] mem_tmp; 
  reg [ 8*5 : 1] str;
  integer transaction_idx; 
  integer i; 
  transaction_idx = 0; 
  mem_tmp [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (distance_c_bitwidth , factor);
  fp = $fopen(`TV_IN_distance ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_distance); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < distance_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
