...
#define AUTOTB_TVOUT_PC_ap_return  "../tv/rtldatafile/rtl.gravity.autotvout_ap_return.dat"
...
class INTER_TCL_FILE {
	public:
		...
		string get_depth_list () {
			...
			total_list << "{distance " << distance_depth << "}\n";
...
float distance)
{
	...
	else
	{
		...
		// "distance"
		char* tvin_distance = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_distance);
		...
		// [[transaction]]
		sprintf(tvin_distance, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_distance, tvin_distance);

		sc_bv<32> distance_tvin_wrapc_buffer;

		// RTL Name: distance
		{
			// bitslice(31, 0)
			{
				// celement: distance(31, 0)
				{
					// carray: (0) => (0) @ (0)
					{
						// sub                   : 
						// ori_name              : distance
						// sub_1st_elem          : 
						// ori_name_1st_elem     : distance
						// regulate_c_name       : distance
						// input_type_conversion : *(int*)&distance
						if (&(distance) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> distance_tmp_mem;
							distance_tmp_mem = *(int*)&distance;
							distance_tvin_wrapc_buffer.range(31, 0) = distance_tmp_mem.range(31, 0);
						}
					}
				}
			}
		...
		for (int i = 0; i < 1; i++)
		{
			sprintf(tvin_distance, "%s\n", (distance_tvin_wrapc_buffer).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_distance, tvin_distance);
		}

		tcl_file.set_num(1, &tcl_file.distance_depth);
		sprintf(tvin_distance, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_distance, tvin_distance);

// [call_c_dut] ---------->

		CodeState = CALL_C_DUT;
		float AESL_return = gravity(m1, m2, distance);
		...
		delete [] tvin_m2;
		// release memory allocation: "distance"
		delete [] tvin_distance;
