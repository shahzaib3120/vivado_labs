...
//------------------------Parameter----------------------
`define TV_IN_address "../tv/cdatafile/c.setMem.autotvin_address.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter address_DEPTH = 1;
reg [31 : 0] address_OPERATE_DEPTH = 1;
parameter address_c_bitwidth = 32;
parameter address_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [address_c_bitwidth - 1 : 0] reg_address;
reg address_write_data_finish;
...
//write address reg
reg [31 : 0] write_address_count = 0;
reg write_address_run_flag = 0;
reg write_one_address_data_done = 0;
...
assign TRAN_AXILiteS_write_data_finish = 1 & address_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        address_write_data_finish <= 0;
        write_address_run_flag <= 0; 
        write_address_count = 0;
        count_operate_depth_by_bitwidth_and_depth (address_c_bitwidth, address_DEPTH, address_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_address_run_flag <= 1; 
            write_address_count = 0;
        end
        if (write_one_address_data_done === 1) begin
            write_address_count = write_address_count + 1;
            if (write_address_count == address_OPERATE_DEPTH) begin
                write_address_run_flag <= 0; 
                address_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_address
    integer write_address_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] address_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = address_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_address_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write address data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (address_c_bitwidth < 32) begin
                        address_data_tmp_reg = reg_address;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < address_c_bitwidth) begin
                                address_data_tmp_reg[j] = reg_address[i*32 + j];
                            end
                            else begin
                                address_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (address_data_in_addr + write_address_count * four_byte_num * 4 + i * 4, address_data_tmp_reg, write_address_resp);
                end
                process_busy = 0;
                write_one_address_data_done <= 1;
                @(posedge clk);
                write_one_address_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
